...
@Tag("Integration tests")
 class UserServiceControllerTest {
    ...
    void shouldGetUserWhenRequestMadeToGetUserByEmail() throws Exception {
        mvc.perform(get(API_V1_USERS_EMAIL + "/abhijeet@gmail.com"))
                .andExpect(status().isOk())
                .andDo(print());
    }
    @Test
    void shouldReturnInternalServerWhenDbReturnsErrorForGetUserByEmail() throws Exception{
        when(userService.getUserDetailsByEmail("abhi@gmail.com")).thenThrow(new UserNotFoundException("abhi@gmail.com"));
        mvc.perform(get(API_V1_USERS_EMAIL+"/abhi@gmail.com"))
                .andExpect(status().isNotFound())
                .andDo(print());
